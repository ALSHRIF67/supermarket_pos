...
<div class="container mx-auto px-4 py-6">
    ...
    <form method="POST" action="{{ route('products.update', $product) }}" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100">
        @csrf
        @method('PUT')
        ...
        <div class="p-6 md:p-8 space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 bg-gray-50 p-6 rounded-2xl border border-gray-100">
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-2 h-[46px] px-4 bg-white border-2 border-dashed border-gray-300 rounded-xl text-green-600 font-bold text-lg" id="profit_display">
                            {{ number_format($product->profit_margin ?? 0, 2) }} ر.س
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        <label for="quantity" class="block text-sm font-bold text-gray-700 mb-1">الكمية المقيدة <span class="text-red-500">*</span></label>
                        <input type="number" name="quantity" id="quantity" value="{{ old('quantity', $product->inventory->quantity ?? 0) }}" 
                    ...
                    <div>
                        ...
                        <input type="number" name="minimum_stock_alert" id="minimum_stock_alert" value="{{ old('minimum_stock_alert', $product->inventory->minimum_stock_alert ?? 5) }}" 
                    ...
                    <div>
                        ...
                        <select name="unit_type" id="unit_type" class="w-full rounded-xl border-gray-200 border-2 focus:border-blue-600 focus:ring-0 transition-colors bg-gray-50/50 focus:bg-white px-4 py-2.5" required>
                            <option value="piece" {{ old('unit_type', $product->inventory->unit_type ?? '') == 'piece' ? 'selected' : '' }}>قطعة</option>
                            <option value="box" {{ old('unit_type', $product->inventory->unit_type ?? '') == 'box' ? 'selected' : '' }}>كرتونة</option>
                            <option value="kg" {{ old('unit_type', $product->inventory->unit_type ?? '') == 'kg' ? 'selected' : '' }}>كيلو جرام</option>
                        </select>
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="date" name="production_date" id="production_date" value="{{ old('production_date', $product->production_date?->format('Y-m-d')) }}" 
                               class="w-full rounded-xl border-gray-200 border-2 focus:border-blue-600 focus:ring-0 transition-colors bg-gray-50/50 focus:bg-white px-4 py-2.5 text-left dir-ltr">
                    </div>
                    <div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        @if($product->product_image)
                            <div class="flex items-center gap-4 mb-3 p-3 bg-gray-50 rounded-xl border border-gray-200">
                                <img src="{{ asset('storage/'.$product->product_image) }}" alt="الصورة الحالية" class="h-16 w-16 object-cover rounded-xl shadow-sm border border-gray-300">
                                <div class="text-sm font-medium text-gray-600">يمكنك رفع صورة جديدة لاستبدال الصورة الحالية.</div>
                            </div>
                        @endif
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="description" id="description" rows="3" 
                                  class="w-full rounded-xl border-gray-200 border-2 focus:border-blue-600 focus:ring-0 transition-colors bg-gray-50/50 focus:bg-white px-4 py-3">{{ old('description', $product->description) }}</textarea>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-gray-50 px-6 py-4 flex flex-col md:flex-row items-center gap-3 border-t border-gray-100">
            <button type="submit" class="w-full md:w-auto px-8 py-3 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition font-bold shadow-md hover:shadow-lg flex justify-center items-center gap-2 text-lg">
                <i class="fas fa-check"></i> تحديث المنتج
            </button>
