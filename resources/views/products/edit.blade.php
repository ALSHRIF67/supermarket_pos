...
@section('content')
    <div class="mb-6">
        <h1 class="text-2xl font-bold">تعديل المنتج: {{ $product->product_name }}</h1>
    </div>
        ...
        <form method="POST" action="{{ route('products.update', $product) }}" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <input type="text" name="product_name" id="product_name" value="{{ old('product_name', $product->product_name) }}" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring focus:ring-blue-200" required>
                ...
                <div>
                    ...
                    <input type="text" name="barcode" id="barcode" value="{{ old('barcode', $product->barcode) }}" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring focus:ring-blue-200" required>
                ...
                <div>
                    ...
                    <input type="text" name="sku" id="sku" value="{{ old('sku', $product->sku) }}" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring focus:ring-blue-200">
                </div>
                <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                <div>
                    ...
                    <input type="number" step="0.01" name="purchase_price" id="purchase_price" value="{{ old('purchase_price', $product->purchase_price) }}" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring focus:ring-blue-200" required>
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="selling_price" id="selling_price" value="{{ old('selling_price', $product->selling_price) }}" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring focus:ring-blue-200" required>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                <div>
                    ...
                    <input type="number" name="quantity" id="quantity" value="{{ old('quantity', $product->inventory->quantity ?? 0) }}" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring focus:ring-blue-200" required>
                    @error('quantity') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label for="minimum_stock_alert" class="block text-sm font-medium text-gray-700 mb-1">حد التنبيه الأدنى</label>
                    <input type="number" name="minimum_stock_alert" id="minimum_stock_alert" value="{{ old('minimum_stock_alert', $product->inventory->minimum_stock_alert ?? 5) }}" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring focus:ring-blue-200">
                </div>
                <div>
                    ...
                    <select name="unit_type" id="unit_type" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring focus:ring-blue-200" required>
                        <option value="piece" {{ old('unit_type', $product->inventory->unit_type ?? 'piece') == 'piece' ? 'selected' : '' }}>قطعة</option>
                        <option value="box" {{ old('unit_type', $product->inventory->unit_type ?? 'piece') == 'box' ? 'selected' : '' }}>كرتونة</option>
                        <option value="kg" {{ old('unit_type', $product->inventory->unit_type ?? 'piece') == 'kg' ? 'selected' : '' }}>كيلو جرام</option>
                    </select>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <input type="date" name="production_date" id="production_date" value="{{ old('production_date', $product->production_date?->format('Y-m-d')) }}" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring focus:ring-blue-200">
                </div>
                <div>
                    <label for="expiry_date" class="block text-sm font-medium text-gray-700 mb-1">تاريخ انتهاء الصلاحية</label>
                    <input type="date" name="expiry_date" id="expiry_date" value="{{ old('expiry_date', $product->expiry_date?->format('Y-m-d')) }}" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring focus:ring-blue-200">
                </div>
                <div>
                    <label for="product_image" class="block text-sm font-medium text-gray-700 mb-1">صورة المنتج</label>
                    @if($product->product_image)
                        <div class="mb-2">
                            <img src="{{ asset('storage/'.$product->product_image) }}" alt="الصورة الحالية" class="h-20 w-20 object-cover rounded">
                        </div>
                    @endif
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" id="description" rows="3" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring focus:ring-blue-200">{{ old('description', $product->description) }}</textarea>
                </div>
            </div>

            <!-- Buttons -->
            <div class="flex items-center gap-4 pt-4 border-t">
                <button type="submit" class="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition">تحديث</button>
